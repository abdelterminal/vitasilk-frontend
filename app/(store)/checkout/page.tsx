...
export default function CheckoutPage() {
    ...
    const openWhatsApp = () => {
        ...
        const lines = cart
            .map(item => `- ${item.product.name} × ${item.quantity} — ${(item.product.price * item.quantity).toLocaleString()} MAD`)
            .join('\n');
        const message = [
            ...
            lines,
            ``,
            `💰 *Total :* ${finalTotal.toLocaleString()} MAD`,
            ``,
            `👤 *Mes coordonnées :*`,
            `- Nom : ${customerInfo.firstName}`,
                                    ...
                                    {step === 1 && (
                                        ...
                                        >
                                            ...
                                            <button
                                                ...
                                                className="mt-4 w-full py-5 bg-[#25D366] hover:bg-[#1ebe5d] text-white text-[10px] uppercase tracking-[0.3em] font-bold transition-all duration-300 flex items-center justify-center gap-3"
                                            >
