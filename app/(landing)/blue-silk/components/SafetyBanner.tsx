...
export function SafetyBanner() {
  const { t } = useLang();
  return (
    // This is the one band that gets a `royal` wash. Elsewhere royal is too dark
    // to do anything (1.85:1 on midnight), but at 30% over navy-light it reads
    // as a deliberate brand-blue lift rather than a colour cast — the sibling
    // themes had to fall back to plain elevation here because their accent went
    // muddy when washed. Text still clears AA comfortably: silk is 12.5:1 on the
    // top stop, silk-dim 8:1.
    <section className="border-y border-gold/25 bg-gradient-to-b from-royal/30 via-navy-light to-navy py-16 md:py-20">
      <div className="mx-auto max-w-5xl px-5">
        <Reveal className="text-center">
          <h2 className="font-display text-3xl text-silk sm:text-4xl">{t.safety.title}</h2>
          <p className="mx-auto mt-4 max-w-xl text-silk-dim">{t.safety.subtitle}</p>
        </Reveal>

        <div className="mt-12 grid gap-6 sm:grid-cols-3">
          {t.safety.items.map((item, i) => (
            <Reveal key={item.title} delay={0.1 * i}>
              <div className="h-full rounded-2xl border-gold-hairline bg-navy-light p-6 text-center shadow-luxe">
