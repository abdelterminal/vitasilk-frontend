...
const DEADLINE_KEY = "vitasilk-blue-offer-deadline";
 ...
 * before the form comes from dropping to navy-deep and lifting the offer card
 * to navy-light — contrast by elevation rather than by inverting the palette.
 */
export function Offer() {
  ...
  return (
    // navy-deep against navy neighbours is only a few points of luminance, so
    // the band needs hairline edges to read as a distinct block.
    <section className="border-y border-gold/20 bg-navy-deep py-20 md:py-28">
      <div className="mx-auto max-w-4xl px-5">
        <Reveal className="text-center">
          <h2 className="font-display text-3xl text-silk sm:text-4xl md:text-5xl">{t.offer.title}</h2>
          <p className="mt-3 text-azure">{t.offer.subtitle}</p>
        </Reveal>

        <Reveal delay={0.15}>
          <OrnamentFrame
            className="mt-12 rounded-3xl bg-gradient-to-b from-navy-light to-navy p-8 sm:p-12"
          >
            <div className="flex flex-col items-center gap-8 md:flex-row md:justify-center md:gap-14">
              ...
              <div className="text-center">
                <h3 className="font-display text-2xl text-silk">{t.offer.unit}</h3>
                <div className="mt-4 flex items-baseline justify-center gap-4">
                  ...
                  <span dir="ltr" className="text-xl text-azure line-through">{formatDh(OLD_PRICE_DH, lang)}</span>
                </div>
                ...
                <ul className="mt-6 space-y-2 text-sm text-azure">
                ...
                {/* countdown */}
                <p className="mt-8 text-xs uppercase tracking-[0.25em] text-azure">{t.offer.countdown.title}</p>
                <div dir="ltr" className="mt-3 flex justify-center gap-3">
                  {left &&
                    (
                      ...
                      <div key={label} className="w-20 rounded-xl border-gold-hairline bg-navy-deep py-3">
                        <span className="block font-display text-3xl text-gold-light tabular-nums">{pad(value)}</span>
                        <span className="mt-1 block text-[10px] uppercase tracking-wider text-azure">{label}</span>
                      </div>
                    ))}
                </div>
                ...
                <a
                  href="#order"
                  className="mt-8 inline-block rounded-full bg-gradient-to-r from-gold via-gold-light to-gold px-10 py-4 text-lg font-semibold text-midnight shadow-lg transition-transform hover:scale-105 active:scale-95 motion-reduce:transition-none"
                >
                  {t.offer.cta}
                </a>
              </div>
