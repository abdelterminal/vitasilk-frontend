...
export function OrderForm() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setStatus("sending");
    try {
      const res = await fetch("/api/landing/blue-silk/order", {
        method: "POST",
  ...
  const inputClass = (error?: string) =>
    `w-full rounded-xl border bg-midnight px-4 py-3.5 text-silk placeholder:text-silk-dim/60 outline-none transition focus:border-gold-light focus:ring-2 focus:ring-gold/30 ${
  ...
  return (
    <section id="order" className="mx-auto max-w-2xl scroll-mt-24 px-5 py-20 md:py-28">
      <Reveal className="text-center">
        <h2 className="font-display text-3xl text-silk sm:text-4xl md:text-5xl">{t.form.title}</h2>
        <p className="mx-auto mt-4 max-w-md text-silk-dim">{t.form.subtitle}</p>
      </Reveal>

      <Reveal delay={0.15}>
        <OrnamentFrame className="mt-10 rounded-3xl bg-navy p-7 shadow-luxe sm:p-10">
          <AnimatePresence mode="wait">
            {status === "success" ? (
              ...
              >
                ...
                <h3 className="font-display text-3xl text-silk">{t.form.successTitle}</h3>
                <p className="mx-auto mt-3 max-w-sm text-silk-dim">{t.form.successText}</p>
              </motion.div>
            ) : status === "error" ? (
              <motion.div
                key="error"
              ...
              >
                ...
                <h3 className="font-display text-2xl text-silk">{t.form.errorTitle}</h3>
                <p className="mx-auto mt-3 max-w-sm text-silk-dim">{t.form.errorText}</p>
                <div className="mx-auto mt-6 max-w-sm space-y-3">
                  <button
                    type="button"
                    onClick={() => setStatus("idle")}
                    className="w-full rounded-full bg-gradient-to-r from-gold via-gold-light to-gold py-3.5 font-semibold text-midnight shadow-lg transition-transform hover:scale-[1.02] active:scale-95 motion-reduce:transition-none"
                  >
                    {t.form.retry}
                  </button>
                  {whatsappLink}
              ...
              <motion.form key="form" exit={{ opacity: 0, y: -10 }} onSubmit={submit} noValidate className="space-y-5">
                <div>
                  <label htmlFor="order-name" className="mb-1.5 block text-sm text-silk-dim">
                    {t.form.name}
                  </label>
                  <input
                    id="order-name"
                ...
                <div>
                  <label htmlFor="order-phone" className="mb-1.5 block text-sm text-silk-dim">
                    {t.form.phone}
                  </label>
                  <input
                    id="order-phone"
                ...
                <div>
                  <label htmlFor="order-city" className="mb-1.5 block text-sm text-silk-dim">
                    {t.form.city}
                  </label>
                  <select
                    id="order-city"
                ...
                <div className="flex items-end justify-between gap-4">
                  <div>
                    <span className="mb-1.5 block text-sm text-silk-dim">{t.form.qty}</span>
                    <div className="flex items-center rounded-xl border border-gold/30 bg-midnight">
                      <button
                        type="button"
                        onClick={() => setQty(Math.max(1, qty - 1))}
                        aria-label="-"
                        className="px-4 py-3 text-xl text-gold-light transition hover:text-silk"
                      >
                        −
                      </button>
                      <span className="w-8 text-center text-lg text-silk tabular-nums">{qty}</span>
                      <button
                        type="button"
                  ...
                  <div className="text-end">
                    <span className="block text-sm text-silk-dim">{t.form.total}</span>
                ...
                <button
                  ...
                  className="w-full rounded-full bg-gradient-to-r from-gold via-gold-light to-gold py-4 text-lg font-semibold text-midnight shadow-lg transition-transform hover:scale-[1.02] active:scale-95 disabled:opacity-60 motion-reduce:transition-none"
                >
