  ...
  return (
    <section className="mx-auto max-w-6xl px-5 py-20 md:py-28">
      <Reveal className="text-center">
        ...
        <h2 className="font-display text-3xl text-silk sm:text-4xl md:text-5xl">{t.beforeAfter.title}</h2>
        <p className="mt-4 text-silk-dim">{t.beforeAfter.subtitle}</p>
      </Reveal>
      ...
      <Reveal delay={0.15}>
        ...
        >
          ...
          <div aria-hidden className="absolute inset-y-0" style={{ left: `${pos}%` }}>
            <div className="absolute inset-y-0 w-0.5 -translate-x-1/2 bg-navy-light shadow-[0_0_0_1px_rgba(154,123,30,0.6)]" />
            <div className="absolute top-1/2 flex h-11 w-11 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-gold-light bg-navy-light/90 text-gold-light shadow-luxe backdrop-blur">
          ...
          {/* labels */}
          <span className="absolute left-3 top-3 rounded-full bg-navy-light/85 px-3 py-1 text-xs font-medium text-silk backdrop-blur">
            {t.beforeAfter.after}
          </span>
          <span className="absolute right-3 top-3 rounded-full bg-navy-light/85 px-3 py-1 text-xs font-medium text-silk backdrop-blur">
