 ...
 * brightness and the band reads as a navy smudge rather than an image. Keeping
 * the top two-thirds untouched lets the argan-grove shot carry the section,
 * while the lower gradient gives the copy something solid to sit on.
 */
export function BrandStory() {
  ...
  return (
    ...
    <section
      ref={ref}
      className="relative min-h-[460px] overflow-hidden bg-navy md:aspect-[16/9] md:min-h-0"
    >
      ...
      {/* midnight veil over the lower half only — enough to seat silk text
          without touching the top two-thirds of the photograph */}
      <div
        aria-hidden
        className="absolute inset-x-0 bottom-0 h-3/5 bg-gradient-to-t from-midnight via-midnight/85 to-transparent"
      />
      ...
      <div className="relative flex h-full min-h-[460px] items-end justify-center px-5 pb-10 md:min-h-0 md:pb-14">
        <Reveal className="max-w-xl text-center">
          {/* silk-dim, not gold: this eyebrow sits over a photograph rather than
              a flat fill, so it needs the higher-contrast neutral. Matches the
              Ingredients eyebrow. */}
          <p className="mb-3 text-sm uppercase tracking-[0.35em] text-silk-dim">
            {t.brandStory.eyebrow}
          </p>
          <h2 className="font-display text-3xl text-silk sm:text-4xl md:text-5xl">
            {t.brandStory.title}
          </h2>
          <p className="mx-auto mt-4 max-w-md leading-relaxed text-silk-dim">
