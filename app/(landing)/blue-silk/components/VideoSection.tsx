    ...
    <section className="bg-midnight py-20 md:py-28">
      <div className="mx-auto max-w-6xl px-5">
        <div className="mb-10 text-center">
          <p className="mb-3 text-sm uppercase tracking-[0.35em] text-silk/60">
            {t.video.eyebrow}
          </p>
          <h2 className="font-display text-4xl text-silk sm:text-5xl">
            {t.video.title}
          </h2>
        </div>
        ...
        <div className="mx-auto max-w-xs sm:max-w-sm">
          <div
            ...
            style={{ boxShadow: "0 20px 60px rgba(0,0,0,0.6), 0 0 0 1px rgba(232,200,106,0.12)" }}
          >
            <video ref={videoRef} muted playsInline loop preload="auto" className="w-full">
              <source src="/blue-silk/video.mp4" type="video/mp4" />
            </video>

            <div style={{ position: "absolute", bottom: "0.75rem", right: "0.75rem", display: "flex", gap: "0.4rem" }}>
              <button onClick={toggleMute} aria-label={muted ? "Unmute" : "Mute"}
                style={{ width: 36, height: 36, borderRadius: "50%", background: "rgba(0,0,0,0.55)", border: "1px solid rgba(232,200,106,0.3)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", backdropFilter: "blur(4px)" }}>
                {muted ? (
              ...
              <button onClick={toggle} aria-label={paused ? "Play" : "Pause"}
                style={{ width: 36, height: 36, borderRadius: "50%", background: "rgba(0,0,0,0.55)", border: "1px solid rgba(232,200,106,0.3)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", backdropFilter: "blur(4px)" }}>
                {paused ? (
                  <svg width="13" height="14" viewBox="0 0 13 14" fill="none"><path d="M2 1.5l9 5-9 5V1.5z" fill="rgba(232,200,106,0.9)" /></svg>
                ) : (
