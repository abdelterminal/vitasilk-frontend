...
const DEADLINE_KEY = "vitasilk-coco-offer-deadline";
...
/**
 * The second deliberately dark section. Dropping to palm-deep here does two
 * things the cream page cannot: it stops the scroll right before the form, and
...
export function Offer() {
  ...
  return (
    <section className="bg-palm-dark py-20 md:py-28">
      <div className="mx-auto max-w-4xl px-5">
        <Reveal className="text-center">
          <h2 className="font-display text-3xl text-cream sm:text-4xl md:text-5xl">{t.offer.title}</h2>
          <p className="mt-3 text-sand">{t.offer.subtitle}</p>
        </Reveal>

        <Reveal delay={0.15}>
          <OrnamentFrame
            tone="dark"
            className="mt-12 rounded-3xl bg-gradient-to-b from-palm-deep to-palm-dark p-8 sm:p-12"
          >
            <div className="flex flex-col items-center gap-8 md:flex-row md:justify-center md:gap-14">
              ...
              <div className="text-center">
                ...
                {/* countdown */}
                <p className="mt-8 text-xs uppercase tracking-[0.25em] text-sand">{t.offer.countdown.title}</p>
                <div dir="ltr" className="mt-3 flex justify-center gap-3">
                  {left &&
                    (
                      ...
                      <div key={label} className="w-20 rounded-xl border-gold-hairline-on-dark bg-palm-dark py-3">
                        <span className="block font-display text-3xl text-gold-light tabular-nums">{pad(value)}</span>
                        <span className="mt-1 block text-[10px] uppercase tracking-wider text-sand">{label}</span>
                      </div>
                    ))}
                </div>
                ...
                <a
                  href="#order"
                  // cta-on-dark, not cta-primary: this is the one CTA on a dark
                  // ground, where the palm fill would drop to 2.6:1. See globals.css.
                  className="mt-8 inline-block rounded-full cta-on-dark px-10 py-4 text-lg font-semibold shadow-lg transition-transform hover:scale-105 active:scale-95 motion-reduce:transition-none"
                >
                  {t.offer.cta}
                </a>
              </div>
