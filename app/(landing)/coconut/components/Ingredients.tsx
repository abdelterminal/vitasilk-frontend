  ...
  return (
    <section className="mx-auto max-w-6xl px-5 py-20 md:py-28">
      <Reveal className="text-center">
        ...
        <p className="mb-3 text-sm uppercase tracking-[0.35em] text-husk">{t.ingredients.eyebrow}</p>
        <h2 className="font-display text-3xl text-bark sm:text-4xl md:text-5xl">{t.ingredients.title}</h2>
        <p className="mx-auto mt-4 max-w-2xl text-husk">{t.ingredients.subtitle}</p>
      </Reveal>
        ...
        <Reveal delay={0.2}>
          <ul className="grid gap-4 sm:grid-cols-2">
            {t.ingredients.items.map((item) => (
              <li
                key={item.name}
                className="rounded-2xl border-gold-hairline bg-shell px-5 py-4"
              >
                <h3 className="font-display text-lg text-bark">{item.name}</h3>
                <p className="mt-1 text-base leading-relaxed text-husk">{item.desc}</p>
              </li>
            ))}
          </ul>
        </Reveal>
