...
export function Hero() {
  ...
  return (
    <header className="relative isolate overflow-hidden bg-gradient-to-b from-cream via-cream to-shell">
      {/* top bar */}
      ...
      {/* sand motes — plain gold dots vanish on cream, so these are warmer
      ...
      {!reduced && (
        <div aria-hidden className="pointer-events-none absolute inset-0 z-0">
          {PARTICLES.map((p, i) => (
            <motion.span
              key={i}
              className="absolute rounded-full bg-sand-deep"
      ...
      <div className="relative z-10 mx-auto grid w-full max-w-6xl items-center gap-10 px-5 pb-20 pt-10 md:grid-cols-2 md:pb-28 md:pt-16">
        ...
        <div className="order-2 text-center md:order-1 md:text-start">
          ...
          <div className="hero-title-wrap">
            <div className="hero-title-inner">
              <motion.h1 {...stagger(1)} className="font-display text-5xl leading-tight sm:text-6xl lg:text-7xl">
                <span className="text-bark">{"Coconut"}</span>{" "}
                <span className="text-gold-shimmer">{"Smooth"}</span>
              </motion.h1>
            </div>
          </div>
          ...
          <motion.div {...stagger(3)} className="mt-8 flex flex-col items-center gap-3 md:flex-row md:items-center md:justify-start">
            <a
              href="#order"
              className="inline-block rounded-full cta-primary px-9 py-4 text-lg font-semibold text-cream animate-glow transition-transform hover:scale-105 active:scale-95 motion-reduce:animate-none motion-reduce:transition-none"
            >
          ...
          <motion.ul {...stagger(4)} className="mt-8 flex flex-wrap justify-center gap-2 md:justify-start">
            {[t.hero.badge1, t.hero.badge2, t.hero.badge3].map((b) => (
              <li
                key={b}
                className="rounded-full border-gold-hairline bg-cream/70 px-4 py-1.5 text-xs text-husk backdrop-blur"
              >
                {b}
              </li>
            ))}
        ...
        <div className="order-1 flex justify-center md:order-2">
          ...
          >
            {/* halo */}
            <div aria-hidden className="product-halo absolute -inset-16 -z-10" />
            <div className="relative animate-float motion-reduce:animate-none">
              ...
              <Image
                ...
                alt={PRODUCT_NAME}
                priority
                className="h-auto w-40 drop-shadow-[0_25px_40px_rgba(36,29,22,0.28)] sm:w-44 md:w-48"
              />
      ...
      <motion.a
        ...
        className="absolute bottom-4 left-1/2 z-10 hidden -translate-x-1/2 text-husk md:block"
