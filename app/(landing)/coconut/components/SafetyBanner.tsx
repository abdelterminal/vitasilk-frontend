...
export function SafetyBanner() {
  const { t } = useLang();
  return (
    <section className="border-y border-gold/25 bg-gradient-to-b from-sand/45 to-shell py-16 md:py-20">
      <div className="mx-auto max-w-5xl px-5">
        <Reveal className="text-center">
          <h2 className="font-display text-3xl text-bark sm:text-4xl">{t.safety.title}</h2>
          <p className="mx-auto mt-4 max-w-xl text-husk">{t.safety.subtitle}</p>
        </Reveal>

        <div className="mt-12 grid gap-6 sm:grid-cols-3">
          {t.safety.items.map((item, i) => (
            <Reveal key={item.title} delay={0.1 * i}>
              <div className="h-full rounded-2xl border-gold-hairline bg-cream p-6 text-center shadow-luxe">
                <span className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-palm/10 text-palm">
